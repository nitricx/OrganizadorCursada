...
```
  [1. ANALYST]           [2. GITFLOW]          [3. DEVELOPER]             [4. QA TESTER]         [5. GITFLOW]
...
### Phase 1: Specification & Ticket Creation (Persona: `analyst`)
...
- **Deterministic Automation**:
  ...
  - Set ready for dev: `npm run ticket:status -- <ISSUE-ID> READY_FOR_DEV`
- **Actions**:
  ...
  6. Advance ticket to `READY_FOR_DEV`: `npm run ticket:status -- <ISSUE-ID> READY_FOR_DEV`.
- **Strict Limitation**: PROHIBITED from editing source files in `src/app/` or executing build/test commands. PROHIBITED from creating local markdown ticket files in the repository (`.agents/tickets/`). All issues MUST strictly reside in GitHub Issues.
- **References**:
  - [`.agents/PERSONAS.md`](./PERSONAS.md)
  - [`.agents/skills/analyst`](./skills/analyst/SKILL.md)
...
- **Deterministic Automation**:
  - Create feature branch: `npm run gitflow:branch -- <ISSUE-ID>`
- **Actions**:
  1. Create semantic branch automatically synced with `develop`:
     ```bash
     npm run gitflow:branch -- <ISSUE-ID>
     ```
...
### Phase 3: Technical Implementation (Persona: `developer`)
...
- **Deterministic Automation**:
  ...
  - Send to QA: `npm run ticket:status -- <ISSUE-ID> READY_FOR_QA`
- **Actions**:
  1. Pick up ticket and mark in progress: `npm run ticket:status -- <ISSUE-ID> IN_DEVELOPMENT developer`.
  ...
  7. Transition state: `npm run ticket:status -- <ISSUE-ID> READY_FOR_QA`.
- **Strict Limitations**:
...
- **References**:
  - [`.agents/PERSONAS.md`](./PERSONAS.md)
  - [`.agents/skills/developer`](./skills/developer/SKILL.md)
