...
async function main() {
  ...
  const tokenArg = token ? `-D sonar.token=${token}` : '';
  const scanCmd = `npx sonar-scanner-npm -D sonar.host.url=${SONAR_HOST} -D sonar.qualitygate.wait=true ${tokenArg}`.trim();

  try {
    execSync(scanCmd, { stdio: 'inherit' });
